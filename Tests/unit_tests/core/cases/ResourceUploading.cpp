...
TEST(ResourceUploading, Chunk)
{
	...
	constexpr std::string_view uploadFileName = "proxy_config.json";

	int errorCode = std::system(std::format(R"(curl --max-time 1800 --insecure --header "Transfer-Encoding: chunked" --header "File-Name: uploaded_{}" --data-binary @{} {})", uploadFileName, uploadFileName, (useHTTPS ? httpsUrl : httpUrl)).data());
...
TEST(ResourceUploading, OctetStream)
{
	...
	int errorCode = std::system(std::format(R"(curl --max-time 1800 --insecure -X POST {} -H "Content-Type: application/octet-stream" -H "File-Name: {}" --data-binary @{})", (useHTTPS ? httpsUrl : httpUrl), uploadFileName, LARGE_FILE_NAME).data());
